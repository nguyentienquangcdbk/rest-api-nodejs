...
const itemController = {
  add: async (req, res) => {
    ...
    try {
      if (!user) {
        ...
        const item = await ItemOrder.find({
          user: user.id,
          color: color,
          size: size,
        });
        if (item.length === 0) {
          ...
          await ItemOrder.findOneAndUpdate(
            { user: user.id, color: req.body.color, size: req.body.size },
            update
          );
  ...
  getAll: async (req, res) => {
    const { id } = req.params;
    try {
      const item = await ItemOrder.find({ user: id });
